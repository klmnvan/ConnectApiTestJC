...
import com.example.apiconnect.api.Result.Error
import com.example.apiconnect.api.Result.Success
import com.example.apiconnect.api.Result.SuccessCode
...
class ViewModelMain(private val repository: Repository): ViewModel() {
    ...
    *
    ...
    private var _state = mutableStateOf<AuthState>(AuthState.Loading)
        ...
        viewModelScope.launch {
            repository.sendCodeEmail(email).collect{
                when(it) {
                    ...
                    is Error -> {
                        //_showErrorToastChannel.send(true)
                        _state.value = AuthState.Success(message = it.message.toString())
                    }
                    is Success -> {
                        _showErrorToastChannel.send(false)
                    }
        ...
        viewModelScope.launch {
            repository.signIn(email, code).collect{
                when(it) {
                    ...
                    is Error -> {
                        //_showErrorToastChannel.send(true)
                        _state.value = AuthState.Error(message = it.message.toString())
                    }
                    is Success -> {
                        _state.value = AuthState.Error(message = it.message.toString())
                    }
